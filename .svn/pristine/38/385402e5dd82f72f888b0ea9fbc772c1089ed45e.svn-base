...
public class ShiroRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) 
		...
		if("admin".equals(username)) {
			credentials = "038bdaf98f2037b31f1e75b5b4c9b26e";    //admin用户的密码经过md5盐值加密后的结果
		}else if("user".equals(username)) {
			credentials = "098d2c478e9c11555ce2823231e02ec1";    //user用户的密码经过md5盐值加密后的结果
		}
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		//注：如果用户是user，则他只有一个user权限；如果用户是admin，则他不仅有user权限，还有admin权限。所以，不同的用户有不同的权限
